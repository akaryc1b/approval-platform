...
const screenshotBlobSha = '4dc0b527a38dde0bf8057222ab4d82e4fc635c5e';
const screenshotLine = '            .runtime/online-demo-image-runtime/*/evaluation-browser-*.png\n';
const delimiter = '\n\n  online-images:\n';
...
// One reviewed retention-only successor; no Action, permission, trigger or job is added.
const screenshotEvolution = Object.freeze({ ...evolution, currentBlobSha: screenshotBlobSha });
const approvedEvolutions = Object.freeze([evolution, screenshotEvolution]);
...
/**
 * Admit the exact additive image job and its exact PNG-retention successor. Both hashes
...
export function projectReviewedWorkflow(path, source, expectedPriorBlobSha = priorBlobSha) {
  ...
  const approved = approvedEvolutions.find(item => item.currentBlobSha === blobSha);
  if (path !== workflowPath || !approved
      || expectedPriorBlobSha !== priorBlobSha) {
    return { priorSource: source, priorBlobSha: blobSha, evolution: null };
  }
  if (blobSha === screenshotBlobSha
      && (source.split(screenshotLine).length !== 2
        || workflowBlobSha(source.replace(screenshotLine, '')) !== currentBlobSha)) {
    throw new Error('reviewed screenshot retention does not preserve the exact image workflow');
  }
  ...
  return { priorSource: `${pieces[0]}\n`, priorBlobSha, evolution: { ...approved } };
}
...
export function reviewedWorkflowDeltas(evolutions = [], workflows) {
  ...
  if (evolutions.length === 0) {
    if (workflows?.some(item => item.path === workflowPath
      && approvedEvolutions.some(approved => approved.currentBlobSha === item.blobSha))) {
  ...
  const approved = approvedEvolutions.find(item => canonical(evolutions[0]) === canonical(item));
  if (!approved) {
    throw new Error('unreviewed workflow evolution evidence');
  }
  if (workflows) {
    const matching = workflows.filter(item => item.path === workflowPath);
    if (matching.length !== 1 || matching[0].blobSha !== approved.currentBlobSha) {
