...
const addition = (index, overrides = {}) => {
  const finding = {
  ...
  finding.findingId = overrides.findingId || canonicalH5OsvFindingId(finding);
  return finding;
};
...
test('H5 OSV extension retains all 117 accepted identities and surfaces canonical additions unresolved', () => {
...
test('H5 OSV extension fails closed on duplicate, incomplete, or forged additions', () => {
  const duplicate = scanner([addition(1), addition(1)]);
  assert.throws(() => reconcileH5OsvFindings(duplicate), /duplicate finding identity/);

  const incomplete = scanner([{ sourceClass: 'E4_OSV_SCANNER', findingId: sha256('incomplete') }]);
  assert.throws(() => reconcileH5OsvFindings(incomplete), /unresolved addition evidence incomplete/);

  const forged = scanner([addition(2, { findingId: 'f'.repeat(64) })]);
  assert.throws(() => reconcileH5OsvFindings(forged), /canonical finding identity mismatch/);
});
