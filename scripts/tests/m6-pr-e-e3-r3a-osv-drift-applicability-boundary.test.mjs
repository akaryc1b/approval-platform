...
const read = (relative) => {
  ...
  assert.equal(existsSync(target), true, `${relative} must exist`);
...
const pluginFixture = () => `The following plugins have been resolved:\n\
...
test('R3A contract is canonical, exact and non-authorizing', () => {
  ...
  assert.deepEqual(contract.findings.map((item) => [
  ...
  ]) assert.equal(contract.decision[key], false, key);
  assert.equal(contract.decision.cumulativeNotApplicableFindings, 4);
  assert.equal(contract.decision.cumulativeUnresolvedFindings, 143);
  assert.equal(contract.decision.releaseBlocked, true);
});

test('R3A parses exact Maven plugin ownership without inventing edges', () => {
  ...
  assert.deepEqual(paths.map((item) => [
    ...
    'MAVEN_RESOLVE_PLUGINS_OWNER_TO_RESOLVED_COMPONENT',
  ...
  ]) assert.ok(paths[0].coResolvedComponents.includes(component), component);
});

test('R3A exact-head resolution covers PR and natural push events', () => {
  const pr = 'a'.repeat(40);
  const push = 'b'.repeat(40);
  const fallback = 'c'.repeat(40);
  assert.equal(resolveExactHead(
    { pull_request: { head: { sha: pr } }, after: push },
    null, fallback, 'true', null,
  ), pr);
  assert.equal(resolveExactHead(
    { after: push }, null, fallback, 'true', null,
  ), push);
  assert.equal(resolveExactHead(
    { head_commit: { id: push } }, null, fallback, 'true', null,
  ), push);
...
test('R3A positively proves Tomcat non-applicability and retains httpcore', () => {
  ...
  assert.equal(
    result.findings[1].evidence.remoteBuildResponsePathProvenUnreachable,
    false,
  );
...
test('R3A documents and permanently imports the non-closure boundary', () => {
  const document = read(
  ...
  assert.doesNotMatch(
    document,
    /PRB_1[67]_(CLOSED|PASSED)|ISSUE_97_(CLOSED|COMPLETED)|M6_PRODUCTION_READINESS_PASSED/,
  );
  assert.ok(read(
  ...
  assert.doesNotMatch(
    read('scripts/security/m6-pr-e-e3-r3a-review-osv-drift.mjs'),
