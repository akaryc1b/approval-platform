...
import { executeEvaluationBrowserRepeat } from './online-demo/evaluation-browser-repeat.mjs';
...
try {
  ...
  if (scope.selected) {
    ...
    try {
      const browser = await executeEvaluationBrowserRepeat({ smoke: result.receipt, directory: result.directory,
        scenario: JSON.parse(readFileSync(resolve(root, 'config/demo/purchase-payment-golden-path.json'), 'utf8')),
        applicationRoots: applications.roots, deadline: started + 42 * 60_000 },
      { rehearse: executeEvaluationBusinessRehearsal });
      if (browser.status !== 'TWO_CLEAN_BROWSER_RUNS_PASSED') throw new Error('BROWSER_REPEAT_REQUIRED');
