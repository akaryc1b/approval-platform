...
Static audit found that `ApprovalMigrationEngineSnapshot` validates only that `snapshotHash` is syntactically a lowercase SHA-256 value. The actual bounded snapshot hash is generated by the existing Flowable adapter protocol:

```text
m5-exact-engine-snapshot-v1
```

Because the outer D4 evidence hash uses `snapshotHash` as an input, relational/payload agreement plus outer evidence-hash recomputation alone is insufficient against coherent mutation of snapshot content, stored `snapshot_hash` and the dependent outer evidence hash.
...
Append-only staged correction history:
...
The test fixture no longer supplies an arbitrary `"a" * 64` hash. `MySqlH5ExactVerificationHashFixture` derives the exact existing Flowable canonical hash, using the same prefix, field ordering and null/blank normalization.

`JdbcApprovalMigrationEngineSnapshotHash` mirrors that existing protocol in persistence infrastructure and is invoked at two exact boundaries:

```text
first finalize:
  requireValid(request.snapshot())
  -> server classification
  -> D4 evidence insert

stored replay read:
  reconstruct typed evidence
  -> requireValid(evidence.snapshot())
  -> D4 request/evidence self-hash checks
  -> exact replay decision
```

A new real-MySQL regression, `coherentlyTamperedStoredSnapshotFailsClosedOnReplay`, first creates genuine H2 -> H3 -> H4 -> H5 exact-target evidence, then changes snapshot content, relational `snapshot_hash`, payload `snapshotHash` and the dependent outer verification evidence hash together. Replay must still fail closed because the inner bounded snapshot content no longer derives to the stored snapshot hash.

Static comparison confirms the Flowable adapter, production verifier and test fixture currently use the same `m5-exact-engine-snapshot-v1` canonical field sequence and the same null/blank-to-empty behavior. No new hash version, Domain classifier or Application contract was introduced.

The previous pre-formal finding is therefore **code-complete on staging but not execution-proven**:
...
- coherent stored snapshot corruption rejection on replay;
...
- first-finalization and replay-time `m5-exact-engine-snapshot-v1` self-proof.
...
Relative to accepted H4 #1432, the expected class-selection delta remains five additional persistence test classes. The new snapshot hash fixture does not change this count. Before actual Run A this is only an expectation, not evidence.
...
snapshot-hash self-proof executable verification
```
