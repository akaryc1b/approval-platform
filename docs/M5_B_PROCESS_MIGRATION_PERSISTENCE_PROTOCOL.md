...
M5-B. This slice adds platform-owned domain and PostgreSQL persistence evidence only.
...
authorize, consume or execute a plan. This slice does not authorize M5-C.
...
Flyway advances continuously from V32 through V35:
...
No V36 or later migration is introduced. The six tenant-scoped tables are:
...
PostgreSQL guards reject deletion, immutable identity changes, revision skipping, prohibited state
transitions, current/event mismatches and mutation of append-only evidence.

## CAS, lineage and UNKNOWN
...
A partial unique index permits at most one active attempt for a tenant-scoped approval instance.
Attempt 1 has no parent. A later attempt must reference the immediately preceding
`FAILED_RETRYABLE` attempt for the same intent and instance. Persistence does not schedule retries.
...
append, active-attempt ownership, retry lineage, immutable gap-free outcome evidence and database
rejection of direct lifecycle bypass.

M5-B remains `IN_PROGRESS` after this first persistence slice. It does not authorize marking PR #58
Ready, merging it, enabling auto-merge, or exposing production migration execution.
