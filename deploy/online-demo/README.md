...
Clients reuse existing pinned upstreams and overlays. Each workspace is bootstrapped once before direct install/build, avoiding a later bootstrap resetting the resolved dependency state. Client bootstrap scripts use Node built-ins and do not install the root workspace, whose lockfile is not committed. Only the generated client workspaces install dependencies; PC keeps its frozen lock. The image receipt records the root policy as `NOT_REQUIRED_FOR_CLIENT_IMAGE_BUILDS`, not as a successfully frozen root install. **H5 retains the existing non-frozen install**; its resolved lock is preserved, but frozen resolution and repeat-build reproducibility are still outstanding. Normalized timestamps and immutable source do not imply identical image bytes.
...
The immutable R2B supply-chain plan remains historical evidence, not a prohibition on the reviewed additive image job. `scripts/security/workflow-evolution.mjs` admits only the exact validation-workflow blob `35f61934083abd086de4833f627568270b91528b`, and proves that removing its single appended job restores byte-exact historical blob `041705d7b2bf148658576b9d5b6d01e64e2d4e9d`. Unrecognized workflow changes still fail closed. The current inventory reports 46 Action occurrences, 15 checkouts and 10 physical jobs; the explicitly historical dependency-graph projection retains 43, 14 and 9 respectively. Current scanners and semantic checks inspect the full current workflow. Historical finding identities, scanner-completion requirements, pin validation and release blockers are not changed.

Run [33960871029](https://github.com/akaryc1b/approval-platform/actions/runs/33960871029) on predecessor `6e30a20` built the backend image but stopped at the unnecessary root frozen install before PC/H5 builds and container startup. Its six supply-chain test failures were workflow-baseline mismatches. Correcting these two causes is not evidence that all application images now build or start; a new exact-candidate run must provide that result.
