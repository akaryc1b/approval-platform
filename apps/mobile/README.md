...
- 审批讨论：待办、已处理、我发起和抄送审批共用评论线程，支持点选 @流程参与人；
- 消息中心：催办、抄送、@提及消息、未读筛选、单条已读和全部已读；
- 审批协作：多选抄送人员、催办当前处理人和逐人已读回执；
- 发起审批模板；
- 个人中心：审批讨论、消息未读入口，以及当前租户、操作人、连接器和 API 环境。

评论附件当前保存平台附件 ID 引用，页面提供引用输入，不代表已经实现文件二进制上传。附件服务接入后会替换为移动文件选择器。
...
- 移动端查询平台投影、评论、消息和审计 API，不读取 Flowable 表；
