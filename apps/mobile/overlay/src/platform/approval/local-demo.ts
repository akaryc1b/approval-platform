...
function launchOperatorId() {
  try {
    const options = uni.getLaunchOptionsSync() as {
      query?: Record<string, string | undefined>
    }
    return options.query?.[operatorQueryParameter]?.trim()
  }
  catch {
    return undefined
  }
...
export function resolveApprovalLocalDemoOperatorId(configuredOperatorId: string) {
  return requireKnownOperator(
    browserOperatorId() || launchOperatorId() || configuredOperatorId,
  )
}
