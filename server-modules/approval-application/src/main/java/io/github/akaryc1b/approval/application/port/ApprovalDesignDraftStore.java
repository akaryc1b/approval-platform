...
/** Tenant-isolated mutable Approval DSL draft repository with optimistic revision updates. */
public interface ApprovalDesignDraftStore {

    void save(ApprovalDesignDraft draft);

    Optional<ApprovalDesignDraft> find(String tenantId, UUID draftId);

    DraftPage findDrafts(DraftCriteria criteria);

    void lock(String tenantId, UUID draftId);

    boolean update(ApprovalDesignDraft draft, long expectedRevision);
