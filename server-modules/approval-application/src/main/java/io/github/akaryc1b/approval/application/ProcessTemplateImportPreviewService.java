...
public final class ProcessTemplateImportPreviewService {
    ...
    public ImportPlan preview(
        ...
        boolean importable = sortedFindings.stream().noneMatch(finding -> finding.severity() == Severity.ERROR)
            && sortedRequirements.stream().noneMatch(RebindingRequirement::required);
        String planHash = hasher.planHash(
            templatePackage.contentHash(), target, sortedFindings, sortedRequirements, acceptedBindings);
        return new ImportPlan(templatePackage.contentHash(), target, sortedFindings,
            sortedRequirements, acceptedBindings, importable, planHash);
    }
    ...
    private static void validateTarget(PreviewRequest request, TenantRegistrySnapshot registry) {
        ...
        version(registry.platformProtocolVersion(), "platformProtocolVersion");
    ...
    private static void checkCompatibility(
        ...
        if (value.manifest().compatibility().supports(target)) {
            findings.add(info("PLATFORM_PROTOCOL_COMPATIBLE", target, "platform protocol is compatible"));
        } else {
    ...
    private static void resolve(
        ...
        switch (dependency.kind()) {
            ...
            case BUSINESS_REFERENCE_TYPE -> bindingDependency(dependency, BindingKind.BUSINESS_REFERENCE,
                registry.businessReferenceTypes(), bindings, findings, requirements);
            case ORGANIZATION_PLACEHOLDER -> bindingDependency(dependency, BindingKind.ORGANIZATION_IDENTITY,
                registry.organizationPlaceholders(), bindings, findings, requirements);
            case IDENTITY_PLACEHOLDER -> bindingDependency(dependency, BindingKind.ORGANIZATION_IDENTITY,
                registry.identityPlaceholders(), bindings, findings, requirements);
    ...
    private static void setDependency(Dependency dependency, Set<String> available, List<Finding> findings) {
    ...
    private static void componentDependency(
        ...
        TenantBinding binding = bindings.get(BindingKind.COMPONENT_IMPLEMENTATION + ":" + dependency.key());
        ...
        ComponentRegistryDescriptor descriptor = templatePackage.componentDescriptors().stream()
            .filter(component -> component.componentKey().equals(dependency.key())).findFirst().orElse(null);
        boolean fallback = descriptor != null
            && descriptor.unsupportedVersionBehavior() == UnsupportedVersionBehavior.READONLY_FALLBACK;
    ...
    private static Finding resolved(Dependency dependency) {
        return info("DEPENDENCY_RESOLVED", dependency.identity(), "dependency is available in target tenant");
    }
    ...
    private static Finding missing(Dependency dependency) {
        return new Finding(dependency.required() ? Severity.ERROR : Severity.WARNING,
            "MISSING_DEPENDENCY", dependency.identity(), "dependency is unavailable in target tenant");
    }
