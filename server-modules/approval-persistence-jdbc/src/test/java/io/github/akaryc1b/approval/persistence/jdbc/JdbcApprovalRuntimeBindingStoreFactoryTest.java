...
class JdbcApprovalRuntimeBindingStoreFactoryTest {

    @Test
    void selectsAcceptedPostgreSqlImplementationFromMetadata() {
        DataSource dataSource = dataSource("PostgreSQL", "16.6", 16, 6);

        assertInstanceOf(
            JdbcApprovalRuntimeBindingStore.class,
            JdbcApprovalRuntimeBindingStoreFactory.create(dataSource)
        );
    }

    @Test
    void selectsMySqlImplementationFromMetadata() {
        DataSource dataSource = dataSource("MySQL", "8.4.4", 8, 4);

        assertInstanceOf(
            JdbcMySqlApprovalRuntimeBindingStore.class,
            JdbcApprovalRuntimeBindingStoreFactory.create(dataSource)
        );
    }
