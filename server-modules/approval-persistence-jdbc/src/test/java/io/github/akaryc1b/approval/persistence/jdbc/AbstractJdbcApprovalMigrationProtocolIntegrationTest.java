...
abstract class AbstractJdbcApprovalMigrationProtocolIntegrationTest {
    ...
    void setUpMigrationProtocol() {
        ...
        jdbc.execute("""
            ...
            """);
        seedReleasePackage(1, hash('b'));
        seedReleasePackage(2, hash('c'));
    ...
    private void seedReleasePackage(int version, String packageHash) {
        int inserted = jdbc.update("""
            ...
            version, hash(hex(version + 7)), "compiler-v1",
            "process-" + version + ".bpmn20.xml", "<definitions/>",
            hash(hex(version + 9)), hash(hex(version + 11)), null, null,
            hash(hex(version + 13)), packageHash, new UUID(70, version),
            "migration-protocol-publisher", offset(NOW.minusSeconds(100 - version))
        );
