...
class JdbcApprovalMessageStoreFactoryTest {

    @Test
    void selectsTheAcceptedPostgreSqlImplementationFromMetadata() {
        DataSource source = dataSource("PostgreSQL", "16.6", 16, 6);

        assertInstanceOf(
            JdbcApprovalMessageStore.class,
            JdbcApprovalMessageStoreFactory.create(
                source,
                new ObjectMapper(),
    ...
    void selectsTheMySqlImplementationFromMetadata() {
        DataSource source = dataSource("MySQL", "8.4.4", 8, 4);

        assertInstanceOf(
            JdbcMySqlApprovalMessageStore.class,
            JdbcApprovalMessageStoreFactory.create(
                source,
                new ObjectMapper(),
                new JdbcTransactionManager(source)
