...
/** Real-FK V38 plan/authorization/consumption provenance for the bounded H1 D5 test. */
final class MySqlH1MigrationPlanAuthorityFixture {

    private static final String AUTHORIZATION_HASH = "7".repeat(64);
    private static final String INTENT_HASH = "8".repeat(64);

    private MySqlH1MigrationPlanAuthorityFixture() {
    }
    ...
    static void seed(
        ...
        ApprovalMigrationPlanAuthorization authorization =
            new ApprovalMigrationPlanAuthorization(
                ...
                AUTHORIZATION_HASH,
                worker,
                "Authorize exact H1 D5 fixture plan",
                "authorization-h1-" + tenant,
                authorizedAt,
                authorizationExpiresAt,
                "request-h1-authorization",
                "trace-h1",
                "audit-event:h1-authorization"
            );
        ...
        requireOne(authorized, "authorize H1 migration plan");
        insertPlanEvent(
            jdbc,
            tenant,
            planId,
        ...
        requireOne(consumed, "consume H1 migration plan");
        insertPlanEvent(
            jdbc,
            tenant,
            planId,
        ...
        int running = jdbc.update("""
            update ap_process_migration_intent set
              status='RUNNING',revision=2,updated_at=?
            where tenant_id=? and intent_id=? and status='PENDING' and revision=1
            """,
            consumedAt.plusSeconds(1),
            tenant,
            intentId.toString()
        );
        requireOne(running, "start H1 migration intent");
        insertIntentEvent(
            jdbc,
            tenant,
            intentId,
    ...
    private static void insertPlanEvent(
        ...
        jdbc.update("""
            ...
            toStatus,
            actor,
            "H1 D5 fixture transition",
            authorizationId == null ? null : authorizationId.toString(),
            authorizationHash,
            happenedAt,
            requestId,
            "trace-h1",
            "audit-event:h1-plan-event-" + revision,
            "{}"
        );
    }
    ...
    private static String writeJson(ObjectMapper objectMapper, Object value) {
        try {
            ...
            throw new IllegalStateException("H1 authority JSON failed", exception);
        }
    }
