...
/** MySQL 8.4 immutable runtime release binding evidence store. */
public final class JdbcMySqlApprovalRuntimeBindingStore
    implements ApprovalRuntimeBindingStore {

    private final NamedParameterJdbcTemplate jdbc;
    private final JdbcDatabaseValueAdapter values;

    public JdbcMySqlApprovalRuntimeBindingStore(DataSource dataSource) {
        ...
        if (values.vendor() != ApprovalDatabaseVendor.MYSQL) {
            throw new IllegalArgumentException(
                "JdbcMySqlApprovalRuntimeBindingStore requires MySQL 8.4"
            );
        }
    ...
    public BindingPage findByRelease(BindingCriteria criteria) {
        BindingCriteria exact = Objects.requireNonNull(
            criteria,
    ...
    public void save(ApprovalRuntimeBinding binding) {
        ApprovalRuntimeBinding exact = Objects.requireNonNull(
            binding,
    ...
    private Optional<ApprovalRuntimeBinding> query(
        ...
        return jdbc.query(
            selectBinding() + " where " + predicate,
            parameters,
            (resultSet, rowNumber) -> binding(resultSet)
        ).stream().findFirst();
    }

    private MapSqlParameterSource parameters(ApprovalRuntimeBinding value) {
        return new MapSqlParameterSource()
            .addValue("tenantId", value.tenantId())
            .addValue("approvalInstanceId", values.bindUuid(value.approvalInstanceId()))
            ...
            .addValue(
                "boundAt",
                values.bindInstant(canonicalInstant(value.boundAt()))
            )
    ...
    private ApprovalRuntimeBinding binding(ResultSet resultSet) throws SQLException {
        return new ApprovalRuntimeBinding(
            resultSet.getString("tenant_id"),
            values.uuid(resultSet, "approval_instance_id"),
            ...
            values.instant(resultSet, "bound_at"),
