...
/** PostgreSQL dynamic add-sign policy and participant evidence store. */
public final class JdbcApprovalTaskCollaborationStore implements ApprovalTaskCollaborationStore {
    ...
    public TaskCollaboration create(TaskCollaboration collaboration) {
        ...
        if (findActiveByTask(collaboration.tenantId(), collaboration.taskId()).isPresent()) {
            throw new CollaborationConflictException(
        ...
        int inserted = jdbc.update(
            ...
            insert into ap_task_collaboration_policy (
                ...
                owner_assignee_id, collaboration_mode, status,
        ...
        for (CollaborationParticipant participant : collaboration.participants()) {
            ...
            if (participantInserted != 1) {
                throw new IllegalStateException("task collaboration participant was not inserted");
            }
        }
        return findByPolicyId(collaboration.tenantId(), collaboration.policyId())
            .orElseThrow(() -> new IllegalStateException(
                "created task collaboration policy disappeared"
            ));
    }
    ...
    public TaskCollaboration removeParticipant(
        ...
        if (current.participants().stream().anyMatch(item ->
            item.status() == ParticipantStatus.APPROVED
                || item.status() == ParticipantStatus.REJECTED
        )) {
            throw new CollaborationConflictException(
                "participants cannot be removed after collaboration decisions begin"
            );
        }
        ...
        if (updated != 1) {
            throw new CollaborationConflictException(
                "collaboration participant changed concurrently"
            );
        }
    ...
    public TaskCollaboration decideParticipant(
        ...
        CollaborationStatus terminal = terminalStatus(afterDecision, normalizedDecision);
        ...
        int policyUpdated = jdbc.update(
            ...
            new MapSqlParameterSource()
                ...
                .addValue("terminalReason", normalizedComment)
        ...
        jdbc.update(
            ...
            new MapSqlParameterSource()
                .addValue("tenantId", afterDecision.tenantId())
                .addValue("policyId", afterDecision.policyId())
                .addValue("canceledAt", offset(normalizedAt))
        );
        return requireByPolicy(afterDecision.tenantId(), afterDecision.policyId());
    }
    ...
    private Optional<TaskCollaboration> findByPolicyId(String tenantId, UUID policyId) {
        List<TaskCollaboration> policies = jdbc.query(
            """
            select *
            from ap_task_collaboration_policy
            where tenant_id = :tenantId
        ...
        TaskCollaboration policy = policies.getFirst();
    ...
    private static CollaborationStatus terminalStatus(
        TaskCollaboration collaboration,
        ParticipantDecision latestDecision
    ) {
        if (latestDecision == ParticipantDecision.REJECTED) {
            return CollaborationStatus.REJECTED;
        }
        if (collaboration.mode() == CollaborationMode.ANY) {
            return CollaborationStatus.SATISFIED;
        }
        boolean pending = collaboration.participants().stream()
            .anyMatch(item -> item.status() == ParticipantStatus.PENDING);
        return pending ? CollaborationStatus.ACTIVE : CollaborationStatus.SATISFIED;
    }
    ...
    private static RowMapper<TaskCollaboration> policyMapper() {
        return (resultSet, rowNumber) -> new TaskCollaboration(
            resultSet.getObject("policy_id", UUID.class),
            ...
            List.of(new CollaborationParticipant(
                UUID.randomUUID(),
                resultSet.getObject("policy_id", UUID.class),
                resultSet.getString("tenant_id"),
                "placeholder",
                new IdentityReference("placeholder", "placeholder", "placeholder"),
