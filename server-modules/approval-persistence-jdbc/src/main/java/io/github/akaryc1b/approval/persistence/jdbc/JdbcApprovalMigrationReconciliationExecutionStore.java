...
/** PostgreSQL D6 durable UNKNOWN reconciliation boundary with no migration redispatch. */
public final class JdbcApprovalMigrationReconciliationExecutionStore
    ...
    public JdbcApprovalMigrationReconciliationExecutionStore(
    ...
    private PreparedReconciliation prepareOnce(PrepareRequest request) {
        ...
        if (prior.isPresent()) {
            ...
            return prepared(request, stored.attempt(), stored.reconciliation(), stored.lease(),
                requestHash, stored);
        }
        ...
        if (attempt.revision() != request.expectedAttemptRevision()
            ...
            throw conflict("attempt is not the exact durable UNKNOWN reconciliation revision");
        }
        requireAmbiguousOutcome(attempt);

        ApprovalMigrationReconciliation reconciliation;
        if (attempt.status() == AttemptStatus.UNKNOWN) {
            if (!protocol.findReconciliations(attempt.tenantId(), attempt.attemptId()).isEmpty()) {
                throw conflict("UNKNOWN attempt already has unexpected reconciliation evidence");
            }
            reconciliation = openReconciliation(attempt, request, requestHash);
            protocol.appendReconciliation(reconciliation);
            ApprovalMigrationAttempt next = attempt.transitioned(new ApprovalMigrationAttemptTransition(
            ...
            attempt = protocol.transitionAttempt(
                next,
                attempt.revision(),
                attemptEvent(attempt, next, request.requestId(), request.traceId())
            );
        } else {
        ...
        if (currentLease.isEmpty()) {
            ...
            if (current.status() != ReconciliationLeaseStatus.ACTIVE
                ...
                throw conflict("reconciliation lease is already active or completed");
            }
            lease = takeOverLease(current, request, requestHash);
            updateLease(current, lease);
            fromStatus = ReconciliationLeaseStatus.ACTIVE;
        }
        insertLeaseEvent(leaseEvent(fromStatus, lease, request.happenedAt()));
        appendAudit(
            attempt.tenantId(),
            request.workerId(),
        ...
        return prepared(request, attempt, reconciliation, lease, requestHash, null);
    }
    ...
    private StoredReconciliation finalizeOnce(FinalizeRequest request) {
        ...
        if (attempt.status() != AttemptStatus.RECONCILING
            ...
            throw conflict("reconciliation attempt, evidence or lease authority is stale");
        }

        ReconciliationDisposition disposition =
            ApprovalMigrationReconciliationObservation.dispositionFor(request.classification());
        ...
        protocol.appendReconciliation(resultReconciliation);
        ...
        ApprovalMigrationReconciliationLease released = releaseLease(lease, request.happenedAt());
    ...
    private ApprovalMigrationReconciliation requireOpenReconciliation(
        ...
        List<ApprovalMigrationReconciliation> values = protocol.findReconciliations(
            attempt.tenantId(),
            attempt.attemptId()
        );
        if (values.isEmpty()) {
        ...
        ApprovalMigrationReconciliation latest = values.get(values.size() - 1);
    ...
    private ApprovalMigrationReconciliation resultReconciliation(
        ...
        switch (observation.disposition()) {
            ...
            default -> throw new IllegalStateException("unsupported reconciliation disposition");
        }
    ...
    private ApprovalMigrationAttempt closeAttemptIfSource(
        ...
        String summary;
        if (observation.disposition() == ReconciliationDisposition.SOURCE_CONFIRMED_NO_RETRY) {
        ...
        return protocol.transitionAttempt(
            next,
            attempt.revision(),
    ...
    private void requireAmbiguousOutcome(ApprovalMigrationAttempt attempt) {
        Integer count = jdbc.queryForObject("""
            ...
            where outcome.tenant_id=:tenantId and outcome.attempt_id=:attemptId
              and request.engine_request_id::text=:requestReference
              and outcome.disposition='AMBIGUOUS_UNKNOWN'
              and outcome.engine_call_may_have_occurred
              and not outcome.engine_call_returned
            """, new MapSqlParameterSource()
                .addValue("tenantId", attempt.tenantId())
                .addValue("attemptId", attempt.attemptId())
                .addValue("requestReference", attempt.engineRequestReference()), Integer.class);
    ...
    private Optional<ApprovalMigrationReconciliationLease> lockLease(
        ...
        return jdbc.query("""
            select payload_json::text from ap_process_migration_reconciliation_lease
            where tenant_id=:tenantId and attempt_id=:attemptId for update
            """, new MapSqlParameterSource()
                .addValue("tenantId", tenantId)
                .addValue("attemptId", attemptId),
            (row, number) -> json.read(row.getString(1), ApprovalMigrationReconciliationLease.class))
            .stream().findFirst();
    }

    private ApprovalMigrationReconciliationLease lockLeaseById(String tenantId, UUID leaseId) {
        return jdbc.query("""
            select payload_json::text from ap_process_migration_reconciliation_lease
            ...
            (row, number) -> json.read(row.getString(1), ApprovalMigrationReconciliationLease.class))
            .stream().findFirst().orElseThrow(() -> conflict("reconciliation lease does not exist"));
    }
    ...
    private ApprovalMigrationReconciliation lockReconciliation(
        ...
        return jdbc.query("""
            ...
            .stream().findFirst().orElseThrow(() -> conflict("reconciliation evidence does not exist"));
    }
    ...
    private void updateLease(
        ...
        int updated = jdbc.update("""
            ...
            where tenant_id=:tenantId and lease_id=:leaseId and revision=:expectedRevision
             and status='ACTIVE'
            """, leaseParameters(next).addValue("expectedRevision", current.revision()));
        if (updated != 1) {
            throw conflict("reconciliation lease revision changed");
        }
    }

    private MapSqlParameterSource leaseParameters(ApprovalMigrationReconciliationLease value) {
        return new MapSqlParameterSource()
            .addValue("tenantId", value.tenantId())
            .addValue("leaseId", value.leaseId())
            ...
            .addValue("leaseUntil", offset(value.leaseUntil()))
            .addValue("acquiredAt", offset(value.acquiredAt()))
            .addValue("updatedAt", offset(value.updatedAt()))
            .addValue("releasedAt", value.releasedAt() == null ? null : offset(value.releasedAt()))
    ...
    private void insertLeaseEvent(ApprovalMigrationReconciliationLeaseEvent event) {
        jdbc.update("""
            insert into ap_process_migration_reconciliation_lease_event (
             ...
             :leaseUntil,:happenedAt,:requestHash,:evidenceHash,:requestId,:traceId,
             cast(:payload as jsonb)
            )
            """, new MapSqlParameterSource()
                .addValue("tenantId", event.tenantId())
                .addValue("eventId", event.eventId())
                .addValue("leaseId", event.leaseId())
    ...
    private void insertObservation(ApprovalMigrationReconciliationObservation value) {
        jdbc.update("""
            ...
            """, new MapSqlParameterSource()
                ...
                .addValue("intentId", value.intentId())
                .addValue("attemptId", value.attemptId())
                .addValue("reconciliationId", value.reconciliationId())
                .addValue("leaseId", value.leaseId())
                .addValue("workerId", value.workerId())
                ...
                .addValue("readSucceeded", value.snapshot().readSucceeded())
    ...
    private Optional<ApprovalMigrationReconciliationObservation> findObservation(
        ...
        return jdbc.query("""
            select payload_json::text from ap_process_migration_reconciliation_observation
    ...
    private StoredReconciliation stored(
        ...
        ApprovalMigrationAttempt attempt = protocol.findAttempt(
            observation.tenantId(),
            observation.attemptId()
        ).orElseThrow(() -> conflict("replayed reconciliation attempt disappeared"));
        ApprovalMigrationReconciliation reconciliation = protocol.findReconciliations(
            observation.tenantId(),
            observation.attemptId()
        ).stream().filter(value -> value.sequence() > 1).reduce((left, right) -> right)
            .orElseThrow(() -> conflict("replayed reconciliation result disappeared"));
        ApprovalMigrationReconciliationLease lease = lockLeaseById(
            observation.tenantId(),
            observation.leaseId()
        );
    ...
    private static void requireReplay(String stored, String requested) {
        if (!stored.equals(requested)) {
            throw conflict("changed-payload migration reconciliation replay is forbidden");
        }
    }
    ...
    private static ReconciliationConflictException conflict(String message, Throwable cause) {
