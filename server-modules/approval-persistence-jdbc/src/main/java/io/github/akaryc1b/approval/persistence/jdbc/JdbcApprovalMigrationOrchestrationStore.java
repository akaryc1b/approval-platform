...
/** PostgreSQL D7 evidence boundary. No method contains an engine call or a long transaction. */
public final class JdbcApprovalMigrationOrchestrationStore
    ...
    public JdbcApprovalMigrationOrchestrationStore(
    ...
    public PreparedOrchestration prepare(PrepareRequest request) {
        ...
        return execute("migration orchestration preparation conflict", () -> prepareOnce(request));
    }

    @Override
    public DispatchAuthorization authorizeDispatch(DispatchRequest request) {
        Objects.requireNonNull(request, "request must not be null");
        return execute("migration dispatch authorization conflict", () -> authorizeOnce(request));
    }

    @Override
    public FinalizedOrchestration finalizeRun(FinalizeRequest request) {
        Objects.requireNonNull(request, "request must not be null");
        return execute("migration orchestration finalization conflict", () -> finalizeOnce(request));
    }

    private PreparedOrchestration prepareOnce(PrepareRequest request) {
        String requestHash = prepareRequestHash(request);
        Optional<OrchestrationRun> replay = findRunByRequestHash(request.tenantId(), requestHash);
        if (replay.isPresent()) {
            ...
            GateDecision gate = canaryGate(run.tenantId(), run.intentId(), canary.approvalInstanceId());
            return prepared(run, canary, gate, latestEvent(run.tenantId(), run.runId()), true);
        }

        PlanContext plan = lockPlan(request.tenantId(), request.intentId());
        advisoryLock(plan.tenantId() + ':' + plan.intentId());
        CanarySelection canary = ensureCanary(plan, request);
        GateDecision gate = canaryGate(plan.tenantId(), plan.intentId(), canary.approvalInstanceId());
    ...
    private DispatchAuthorization authorizeOnce(DispatchRequest request) {
        ...
        advisoryLock(run.tenantId() + ':' + run.runId());
        ...
        if (prior.isPresent()) {
            ...
            return new DispatchAuthorization(
                observation,
                findAttemptEvent(stored.tenantId(), stored.runId(), request.attemptId()),
        ...
        appendAudit(stored,
            ...
            Map.of(
                ...
                "expectedKillSwitchRevision", Long.toString(request.expectedKillSwitchRevision()),
                "observedKillSwitchRevision", Long.toString(observed.revision()),
                "result", reasonCode
            ));
    ...
    private FinalizedOrchestration finalizeOnce(FinalizeRequest request) {
        ...
        advisoryLock(supplied.tenantId() + ':' + supplied.runId());
    ...
    private CanarySelection ensureCanary(PlanContext plan, PrepareRequest request) {
        Optional<CanarySelection> existing = findCanaryOptional(plan.tenantId(), plan.planId());
        ...
        jdbc.update("""
            ...
            """, new MapSqlParameterSource()
                ...
                .addValue("selectionId", canary.selectionId())
                .addValue("planId", canary.planId())
                .addValue("intentId", canary.intentId())
                .addValue("algorithmVersion", canary.algorithmVersion())
                .addValue("sequenceNo", canary.sequenceNo())
                .addValue("instanceId", canary.approvalInstanceId())
                ...
                .addValue("recordedAt", offset(canary.recordedAt()))
    ...
    private CompletionDecision completionDecision(
        ...
        if (run.phase() == OrchestrationPhase.CANARY) {
            return new CompletionDecision(RunEventType.CANARY_COMPLETED, PauseReason.NONE, false);
        }
        ...
        boolean allCompleted = selected != null && completed != null && selected.equals(completed);
    ...
    private static AttemptFact fact(
        ...
        if (completed && "SUCCEEDED".equals(status) && !conflicted) {
            ...
            "BLOCKED_STALE", "FAILED_RETRYABLE", "FAILED_TERMINAL", "CANCELLED"
        ).contains(status)) {
    ...
    private void requireClaimedAttempt(OrchestrationRun run, UUID attemptId, Instant happenedAt) {
        Integer count = jdbc.queryForObject("""
            ...
            """, new MapSqlParameterSource()
                ...
                .addValue("intentId", run.intentId())
                .addValue("attemptId", attemptId)
                .addValue("happenedAt", offset(happenedAt)), Integer.class);
    ...
    private void insertRun(OrchestrationRun run) {
        jdbc.update("""
            insert into ap_process_migration_orchestration_run (
             ...
             :runEvidenceHash,:startedAt,:requestId,:traceId,cast(:payload as jsonb)
            )
            """, new MapSqlParameterSource()
                .addValue("tenantId", run.tenantId())
                .addValue("runId", run.runId())
                .addValue("planId", run.planId())
                .addValue("intentId", run.intentId())
                .addValue("runRevision", run.runRevision())
                .addValue("phase", run.phase().name())
                .addValue("requestedLimit", run.requestedLimit())
                .addValue("canarySelectionId", run.canarySelectionId())
                ...
                .addValue("startedAt", offset(run.startedAt()))
    ...
    private void insertObservation(KillSwitchObservation observation) {
        jdbc.update("""
            insert into ap_process_migration_kill_switch_observation (
             ...
             :observedAt,:requestId,:traceId,cast(:payload as jsonb)
            )
            """, new MapSqlParameterSource()
                .addValue("tenantId", observation.tenantId())
                .addValue("observationId", observation.observationId())
                .addValue("runId", observation.runId())
                .addValue("attemptId", observation.attemptId())
                ...
                .addValue("observedAt", offset(observation.observedAt()))
    ...
    private void insertBatch(BoundedBatch batch) {
        jdbc.update("""
            insert into ap_process_migration_orchestration_batch (
             ...
             cast(:attemptIds as jsonb),cast(:dispositions as jsonb),:predecessorHash,:batchHash,
             :recordedAt,:requestId,:traceId,cast(:payload as jsonb)
            )
            """, new MapSqlParameterSource()
                .addValue("tenantId", batch.tenantId())
                .addValue("batchId", batch.batchEvidenceId())
                .addValue("runId", batch.runId())
                .addValue("claimBatchId", batch.claimBatchId())
                ...
                .addValue("recordedAt", offset(batch.recordedAt()))
    ...
    private OrchestrationEvent appendEvent(
        ...
        jdbc.update("""
            insert into ap_process_migration_orchestration_event (
             ...
             :predecessorHash,:eventHash,:happenedAt,:requestId,:traceId,cast(:payload as jsonb)
            )
            """, new MapSqlParameterSource()
    ...
    private Optional<OrchestrationRun> findRunByRequestHash(String tenantId, String requestHash) {
        return queryOne(
            "select payload_json::text from ap_process_migration_orchestration_run "
    ...
    private Optional<CanarySelection> findCanaryOptional(String tenantId, UUID planId) {
        return queryOne(
            "select payload_json::text from ap_process_migration_canary_selection "
                + "where tenant_id=:tenantId and plan_id=:planId",
    ...
    private OrchestrationEvent latestEvent(String tenantId, UUID runId) {
        return queryOne(
            "select payload_json::text from ap_process_migration_orchestration_event "
                + "where tenant_id=:tenantId and run_id=:runId order by sequence desc limit 1",
            params("tenantId", tenantId, "runId", runId),
            OrchestrationEvent.class
        ).orElseThrow(() -> conflict("orchestration event lineage was not found"));
    }

    private OrchestrationEvent findAttemptEvent(String tenantId, UUID runId, UUID attemptId) {
        return queryOne(
            "select payload_json::text from ap_process_migration_orchestration_event "
                + "where tenant_id=:tenantId and run_id=:runId and attempt_id=:attemptId "
                + "order by sequence desc limit 1",
            params("tenantId", tenantId, "runId", runId, "attemptId", attemptId),
            OrchestrationEvent.class
    ...
    private Optional<KillSwitchObservation> findObservation(
        ...
        return queryOne(
            "select payload_json::text from ap_process_migration_kill_switch_observation "
                + "where tenant_id=:tenantId and run_id=:runId and attempt_id=:attemptId",
            params("tenantId", tenantId, "runId", runId, "attemptId", attemptId),
    ...
    private Optional<BoundedBatch> findBatch(String tenantId, UUID runId) {
        return queryOne(
            "select payload_json::text from ap_process_migration_orchestration_batch "
                + "where tenant_id=:tenantId and run_id=:runId",
            params("tenantId", tenantId, "runId", runId),
            BoundedBatch.class
        );
    ...
    private long currentRunRevision(String tenantId, UUID intentId) {
        Long value = jdbc.queryForObject(
            "select coalesce(max(run_revision),0) from ap_process_migration_orchestration_run "
                + "where tenant_id=:tenantId and intent_id=:intentId",
            params("tenantId", tenantId, "intentId", intentId),
            Long.class
        );
    ...
    private String latestRunHash(String tenantId, UUID intentId) {
        ...
        ).stream().findFirst().orElseThrow(() -> conflict("run predecessor was not found"));
    }

    private long currentEventSequence(String tenantId, UUID runId) {
        Long value = jdbc.queryForObject(
            "select coalesce(max(sequence),0) from ap_process_migration_orchestration_event "
                + "where tenant_id=:tenantId and run_id=:runId",
            params("tenantId", tenantId, "runId", runId),
            Long.class
        );
    ...
    private static boolean terminalEvent(RunEventType type) {
        return switch (type) {
            ...
            case KILL_SWITCH_BLOCKED, CANARY_COMPLETED, BATCH_RECORDED, PAUSED, COMPLETED -> true;
        };
    }
    ...
    private void advisoryLock(String key) {
        jdbc.query(
            ...
            new MapSqlParameterSource().addValue("lockKey", key),
            resultSet -> null
        );
    }
    ...
    private static OrchestrationConflictException conflict(String message, Throwable cause) {
