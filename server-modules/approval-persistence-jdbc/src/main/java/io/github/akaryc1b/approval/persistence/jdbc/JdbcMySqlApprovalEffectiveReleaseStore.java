...
/** MySQL 8.4 effective release projection with revision CAS and immutable history. */
public final class JdbcMySqlApprovalEffectiveReleaseStore
    ...
    public JdbcMySqlApprovalEffectiveReleaseStore(DataSource dataSource) {
        ...
        if (values.vendor() != ApprovalDatabaseVendor.MYSQL) {
            throw new IllegalArgumentException(
                "JdbcMySqlApprovalEffectiveReleaseStore requires MySQL 8.4"
            );
        }
    ...
    public void save(
        ...
        ApprovalEffectiveRelease exact = Objects.requireNonNull(
            effectiveRelease,
            "effectiveRelease must not be null"
        );
        ApprovalEffectiveRelease.Activation evidence = Objects.requireNonNull(
            activation,
    ...
    public boolean update(
        ...
        if (exact.revision() != expectedRevision + 1) {
            throw new IllegalArgumentException(
                "effective release revision must advance exactly once"
            );
        }
        int updated = jdbc.update(
            ...
            """,
            currentParameters(exact).addValue("expectedRevision", expectedRevision)
        );
        if (updated == 0) {
            return false;
        }
        if (updated != 1) {
            throw new IllegalStateException("effective release update was not singular");
        }
        insertActivation(evidence);
        return true;
    }
    ...
    private MapSqlParameterSource currentParameters(ApprovalEffectiveRelease value) {
        return new MapSqlParameterSource()
            ...
            .addValue("activatedAt", values.bindInstant(canonicalInstant(value.activatedAt())))
            .addValue("changeReason", value.changeReason())
            .addValue("requestId", value.requestId())
            .addValue("traceId", value.traceId());
    }

    private MapSqlParameterSource activationParameters(
        ApprovalEffectiveRelease.Activation value
    ) {
        return new MapSqlParameterSource()
            .addValue("activationId", values.bindUuid(value.activationId()))
            .addValue("tenantId", value.tenantId())
            .addValue("definitionKey", value.definitionKey())
    ...
    private ApprovalEffectiveRelease current(ResultSet resultSet) throws SQLException {
        return new ApprovalEffectiveRelease(
            ...
            values.instant(resultSet, "activated_at"),
    ...
    private ApprovalEffectiveRelease.Activation activation(ResultSet resultSet)
        throws SQLException {
        return new ApprovalEffectiveRelease.Activation(
            values.uuid(resultSet, "activation_id"),
            resultSet.getString("tenant_id"),
            resultSet.getString("definition_key"),
    ...
    private static void requireMatchingEvidence(
        ApprovalEffectiveRelease release,
        ApprovalEffectiveRelease.Activation activation
    ) {
        if (!release.tenantId().equals(activation.tenantId())
            || !release.definitionKey().equals(activation.definitionKey())
            || release.effectiveReleaseVersion() != activation.releaseVersion()
            || !Objects.equals(
                release.previousReleaseVersion(),
                activation.previousReleaseVersion()
            )
            || !release.releasePackageHash().equals(activation.releasePackageHash())
            || release.definitionVersion() != activation.definitionVersion()
            || release.formPackageVersion() != activation.formPackageVersion()
            || !release.compilerVersion().equals(activation.compilerVersion())
            || !release.engineDeploymentId().equals(activation.engineDeploymentId())
            || !release.engineDefinitionId().equals(activation.engineDefinitionId())
            || release.engineVersion() != activation.engineVersion()
            || release.revision() != activation.revision()
            || !release.activatedBy().equals(activation.activatedBy())
            || !release.activatedAt().equals(activation.activatedAt())
            || !release.changeReason().equals(activation.changeReason())
            || !release.requestId().equals(activation.requestId())
            || !Objects.equals(release.traceId(), activation.traceId())) {
            throw new IllegalArgumentException(
                "effective release and activation evidence do not match"
            );
        }
    }
