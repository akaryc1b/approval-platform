...
final class JdbcMySqlTransactionLockManager {
    ...
    void acquire(String scope) {
        ...
        if (existing != null) {
            existing.acquire(exactScope, timeoutSeconds);
            return;
        }
        ...
        boolean bound = false;
        try {
            created.acquire(exactScope, timeoutSeconds);
    ...
    private static final class LockRegistry {
        ...
        private LockRegistry(Connection connection) {
            ...
            this.databaseNamespace = databaseNamespace(connection);
        }

        private void acquire(String scope, int timeoutSeconds) {
            String lockName = sha256(LOCK_NAMESPACE + databaseNamespace + ':' + scope);
        ...
        private static String databaseNamespace(Connection connection) {
            try {
                return requireText(connection.getCatalog(), "database catalog");
            } catch (SQLException exception) {
                throw new IllegalStateException(
                    "MySQL projection lock database identity resolution failed",
                    exception
                );
            }
        }
