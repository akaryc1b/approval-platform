...
/** MySQL 8.4 read-only repeatable-read aggregation over immutable V49 evidence. */
public final class JdbcMySqlApprovalAssistanceGovernanceHistoryQuery
    ...
    public JdbcMySqlApprovalAssistanceGovernanceHistoryQuery(
        ...
        if (values.vendor() != ApprovalDatabaseVendor.MYSQL) {
            throw new IllegalArgumentException(
                "JdbcMySqlApprovalAssistanceGovernanceHistoryQuery requires MySQL 8.4"
            );
        }
    ...
    private HistorySummary summarizeOnce(HistoryWindow window) {
        ...
        SummaryRow summary = jdbc.query("""
            select count(*) as total_evidence,
                   coalesce(sum(case when s.state='ACTIVE' then 1 else 0 end),0)
                     as active_evidence,
                   coalesce(sum(case when s.state='TOMBSTONED' then 1 else 0 end),0)
                     as tombstoned_evidence,
                   coalesce(sum(case when e.provider_invocation_started then 1 else 0 end),0)
                     as provider_invocation_count,
                   coalesce(sum(e.provider_attempts),0) as provider_attempt_count,
                   coalesce(sum(case when e.advisory_result_present then 1 else 0 end),0)
                     as advisory_result_count,
                   coalesce(sum(case when e.retry_attempted then 1 else 0 end),0)
                     as unsafe_retry_count,
                   coalesce(
                   ...
                   coalesce(
                     sum(
                       case when s.state='ACTIVE'
                         and e.retention_until<=:observedAt
                         then 1 else 0 end
                     ),
                     0
        ...
        jdbc.query("""
            ...
            """, parameters, (result, rowNumber) -> {
                ...
                long distinctVersions = result.getLong(
                    "distinct_version_bundle_count"
                );
    ...
    private SummaryRow summaryRow(ResultSet result) throws SQLException {
    ...
    private MapSqlParameterSource parameters(HistoryWindow window) {
        return new MapSqlParameterSource()
            .addValue("tenantId", window.tenantId())
            .addValue("fromInclusive", values.bindInstant(window.fromInclusive()))
            .addValue("toExclusive", values.bindInstant(window.toExclusive()))
            .addValue("observedAt", values.bindInstant(window.observedAt()));
    }
