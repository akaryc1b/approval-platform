...
if [ ! -f .github/pr53-connected.patch ]; then
...
def apply_exact(patch: Path) -> tuple[int, int]:
    ...
    while index < len(lines):
        ...
        while index < len(lines) and not lines[index].startswith('diff --git '):
            ...
            target_offset = line_offsets[
                min(max(old_start - 1, 0), len(line_offsets) - 1)
            ]
            distances = sorted((abs(value - target_offset), value) for value in candidates)
            if len(distances) > 1 and distances[0][0] == distances[1][0]:
                raise SystemExit(
                    f'Exact patch context ambiguous for {path} near line {old_start}'
                )
        ...
        if file_hunks:
            ...
            changed_files += 1
    return changed_files, changed_hunks

files = 0
hunks = 0
for patch_name in (
    '.github/pr53-connected.patch',
    '.github/pr53-connected-fix.patch',
):
    patch = Path(patch_name)
    if patch.is_file():
        patch_files, patch_hunks = apply_exact(patch)
        files += patch_files
        hunks += patch_hunks
print(f'Applied {hunks} exact hunks across {files} file applications')
PY
